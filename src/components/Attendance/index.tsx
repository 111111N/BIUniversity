...
export const AttendanceTable: React.FC<AttendanceTableProps> = ({ students, setStudents }) => {
  ...
  const [activeCell, setActiveCell] = useState<{ row: number | null; col: number | null }>({
    row: null,
    col: null,
  });
  ...
  const startEditing = () => {
    ...
    setOriginalStudents(
      normalized.map(s => ({ ...s, attendance: { ...s.attendance }, grades: { ...s.grades } }))
    );
    setDraftStudents(
      normalized.map(s => ({ ...s, attendance: { ...s.attendance }, grades: { ...s.grades } }))
    );
  ...
  const calcAverage = (student: Student) => {
    const gradesArray = workDays
      ...
      .filter((g): g is Exclude<Grade, null> => g !== null); // исключаем null
  ...
  return (
    <div className={styles.attendanceTableContainer}>
      <div className={styles.layoutWithControls}>
        <h2>Таблица {mode === 'attendance' ? 'посещаемости' : 'оценок'}</h2>
  ...
  {grade ?? ''}
        ...
        <div className={styles.controls}>
          ...
          <div className={styles.rightButtons}>
            ...
            <button onClick={isEditing ? acceptChanges : startEditing} className={styles.controlButton}>
              {isEditing ? (
                <AcceptIcon size={24} color="var(--color-light)" fill="var(--color-light)" />
              ) : (
                <EditIcon size={24} color="var(--color-light)" />
              )}
            </button>
          </div>
        </div>
